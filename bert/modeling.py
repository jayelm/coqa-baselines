...
class BertForQuestionAnswering(PreTrainedBertModel):
    ...
    ```
    """
    def __init__(self, config, use_history=False):
        ...
        if use_history:
            N_HISTORY_F = 82
            self.qa_outputs = nn.Linear(config.hidden_size + N_HISTORY_F, 2)
        else:
            self.qa_outputs = nn.Linear(config.hidden_size, 2)
        self.use_history = use_history
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, f_history=None, debug=False):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        if self.use_history:
            sequence_output = torch.cat((sequence_output, f_history), 2)
